<?php
/**
 * filters.php -  Nagios SNMP Trap Interface
 * 
 * PHP Version 5.2
 * 
...
class filters extends frontend {
    ...
    **/
    function route_request() {
        if (DEBUG&&DEBUGLEVEL&1) debug('Start method filters::route_request()');
        ...
        switch($mode) {
            case 'delete':
                ...
                $this->DATABASE->deleteItem('filters',$filterid);
                $this->constructorViewMain();
                break;
            case 'viewfilter':
                $status = 'disabled';
                $filter = $this->DATABASE->getItem('filters',$filterid);
                $this->constructorEditMain($filter,$status);
                break;            
            case 'Save':
                $formarray = grab_request_var('filters');
                $this->DATABASE->saveForm($formarray,$filterid);
                $this->constructorViewMain();
                break;
            case 'view':
                $this->constructorViewMain();
            ...
            if (DEBUG&&DEBUGLEVEL&1) debug('End method filters::route_request();');
    }
